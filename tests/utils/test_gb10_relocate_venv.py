...
def test_relocate_retargets_dangling_uv_python(tmp_path: Path):
    ...
    uv_home = (
        "/home/roccen/.local/share/uv/python/"
        "cpython-3.12.12-linux-aarch64-gnu/bin"
    )
...
def test_relocate_retargets_usr_bin_python3(tmp_path: Path):
    """Spark host venv: bin/python -> /usr/bin/python3 (not python3.12)."""
    venv = tmp_path / "opt" / "venv"
    src = tmp_path / "opt" / "vllm"
    (venv / "bin").mkdir(parents=True)
    (src / "vllm").mkdir(parents=True)
    ...
    _run_relocate(venv, src, "/home/roccen/.venvs/vllm028", "/home/roccen/src/vllm")
