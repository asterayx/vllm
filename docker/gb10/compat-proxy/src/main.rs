...
struct Args {
    ...
    display_name: String,
    /// Context limit advertised in generated client configurations.
    #[arg(
        long,
        default_value_t = 524288,
        env = "SPARK_COMPAT_CONTEXT_WINDOW",
        global = true
    )]
    context_window: u32,
    #[command(subcommand)]
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let public_base = args
        .public_base
        .unwrap_or_else(|| default_public_base(&args.listen));
    ...
    futures_util::stream::poll_fn(move |cx| {
        ...
        match stream.poll_next_unpin(cx) {
            std::task::Poll::Ready(Some(Ok(chunk))) => std::task::Poll::Ready(Some(Ok(
                Bytes::from(rewrite_sse_chunk(&mut pending, &chunk)),
            ))),
