...
"""SM12x Triton FP8 einsum for DeepSeek V4 o_proj.

Compiled DeepGEMM ``fp8_einsum`` asserts on SM12x
(``einsum.hpp: m == m_ and n == n_ and k == k_`` / ``layout.hpp`` SF).
Use the SM90-shaped ``(1, 128, 128)`` recipe plus this kernel. Ported from
jasl/vllm via vLLM #52357 / #43743.
"""
...
def _e8m0_like_dtypes() -> tuple[torch.dtype, ...]:
    dtypes: list[torch.dtype] = [torch.uint8]
...
def _fp32_block_scales(scale: torch.Tensor) -> torch.Tensor:
    if scale.dtype in _e8m0_like_dtypes():
...
def deepseek_v4_sm12x_fp8_einsum(
    ...
    ``[tokens, groups, hidden]``. ``b`` is ``wo_a`` as
    ...
    a_scale = _fp32_block_scales(a_scale)
    b_scale = _fp32_block_scales(b_scale)
...
def _use_deepseek_v4_sm12x_triton_fp8_einsum(
    ...
    return (
        ...
        and b_scale.dtype in (torch.float32, *_e8m0_like_dtypes())
    )


def _reshape_wo_a_for_einsum(
    b: torch.Tensor,
    b_scale: torch.Tensor,
    out: torch.Tensor,
    a: torch.Tensor,
    ...
    if b_scale.dim() == 2:
        ...
        if b_scale.shape[0] % scale_out_blocks != 0:
            raise RuntimeError(
                ...
                f"scale_out_blocks={scale_out_blocks}, got {b_scale.shape[0]}"
            )
...
def deepseek_v4_fp8_einsum(
    ...
    b, b_scale = _reshape_wo_a_for_einsum(b, b_scale, out, a, recipe)
